...
expect.extend({
    toBeExactly025Inches(received) {
        ...
        if (pass) {
            ...
            return {
                message: () => `Expected ${received} to be exactly 0.25 inches (EPA CGP requirement). ` +
                    `Received: ${received}. This MUST be exact to avoid regulatory violations.`,
                pass: false,
            };
        }
    },
    toBeWithin24Hours(received, baseDate = new Date()) {
        ...
        if (pass) {
            ...
            return {
                message: () => `Expected ${received.toISOString()} to be within 24 hours of ${baseDate.toISOString()}. ` +
                    `Difference: ${diffHours.toFixed(2)} hours. EPA requires inspection within 24 hours.`,
                pass: false,
            };
        }
    },
    toBeValidGPSCoordinates(received) {
        ...
        if (pass) {
            ...
            return {
                message: () => `Expected valid GPS coordinates but received {lat: ${received.lat}, lng: ${received.lng}}. ` +
...
globals_1.jest.mock('@clerk/nextjs', () => ({
...
globals_1.jest.mock('@capacitor/core', () => ({
    Capacitor: {
        isNativePlatform: () => false,
        getPlatform: () => 'web'
    }
}));
globals_1.jest.mock('@capacitor/camera', () => ({
    Camera: {
        getPhoto: globals_1.jest.fn(() => Promise.resolve({
...
globals_1.jest.mock('@capacitor/geolocation', () => ({
    Geolocation: {
        getCurrentPosition: globals_1.jest.fn(() => Promise.resolve({
            coords: {
                latitude: 40.7128,
                longitude: -74.0060,
                accuracy: 10
...
globals_1.jest.mock('axios', () => ({
    default: {
        get: globals_1.jest.fn(() => Promise.resolve({
            data: {
                main: {
                    temp: 72,
                    humidity: 65
        ...
        post: globals_1.jest.fn(() => Promise.resolve({ data: {} })),
        create: globals_1.jest.fn(() => ({
            get: globals_1.jest.fn(() => Promise.resolve({ data: {} })),
            post: globals_1.jest.fn(() => Promise.resolve({ data: {} }))
        }))
    }
}));
