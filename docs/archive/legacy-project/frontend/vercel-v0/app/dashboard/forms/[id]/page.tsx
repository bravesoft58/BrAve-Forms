...
// Interface for related forms grouped by type
interface RelatedFormsByType {
  type: string
  forms: Array<{
    id: string
    date: string
...
export default function FormDetailPage({ params }: { params: { id: string } }) {
  ...
  const [relatedFormsByType, setRelatedFormsByType] = useState<RelatedFormsByType[]>([])
  ...
  const fetchForm = async () => {
    ...
    const formsByType: Record<string, Array<{ id: string; date: string }>> = {}
    ...
    const relatedFormsByType: RelatedFormsByType[] = Object.keys(formsByType).map((type) => ({
      type,
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const { downloadFormPDF } = await import("@/lib/pdf-generator")
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Submitted on {form.submittedDate}</CardDescription>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" size="icon" asChild>
                    <Link href={`/dashboard/forms/${form.id}/qr`}>
              ...
              <div className="space-y-6">
                ...
                <div className="grid gap-2">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <Link href={`/dashboard/projects/${form.project.id}`} className="hover:underline">
                      {form.project.name}
                    </Link>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <ul className="mt-2 list-disc pl-5 text-sm text-muted-foreground">
                          {form.content.dustControlMeasures?.map((measure, index) => (
                  ...
                  {form.type === "SWPPP Inspection" && (
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <ul className="mt-2 list-disc pl-5 text-sm text-muted-foreground">
                          {form.content.bmpConditions?.map((condition, index) => (
                  ...
                  {form.type === "Safety Inspection" && (
                    <div className="space-y-4">
                      <div>
                        ...
                        <ul className="mt-2 list-disc pl-5 text-sm text-muted-foreground">
                          {form.content.findings?.map((finding, index) => (
          ...
          {form.hasPhotos && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Photos attached to this form</CardDescription>
              </CardHeader>
              <CardContent>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Previous versions of this form</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Forms for {form.project.name}</CardDescription>
            </CardHeader>
            <CardContent>
              {relatedFormsByType.length > 0 ? (
                <div className="space-y-6">
                  {relatedFormsByType.map((formType) => (
                    <div key={formType.type} className="space-y-2">
                      <div className="flex items-center gap-2">
                        <FolderOpen className="h-4 w-4 text-primary" />
                        <h3 className="text-sm font-medium">{formType.type}</h3>
                      </div>
                      <div className="ml-6 space-y-2 border-l pl-4">
                        {formType.forms.map((relatedForm) => (
                          <div key={relatedForm.id} className="flex items-center justify-between text-sm">
                            <div className="flex items-center gap-2">
                              <FileText className="h-4 w-4 text-muted-foreground" />
                              <span>{relatedForm.date}</span>
                            </div>
                            <Button variant="link" size="sm" className="h-auto p-0" asChild>
                              <Link href={`/dashboard/forms/${relatedForm.id}`}>View</Link>
                            </Button>
                          </div>
                        ))}
                      </div>
                ...
                <div className="text-sm text-muted-foreground text-center py-4">No other forms for this project</div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Add notes or comments about this form</CardDescription>
            </CardHeader>
            <CardContent>
              <FormComment formId={form.id} />
            </CardContent>
          </Card>
        </div>
      </div>
