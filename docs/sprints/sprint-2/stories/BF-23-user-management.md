...
- [x] AC-1: Users page shows table of all users (name, email, role, created date)
- [x] AC-2: Admin can create a new user (email, name, role) — sends invite email
- [x] AC-3: Admin can delete a user (with confirmation)
- [x] AC-4: Admin can change a user's role (admin ↔ user)
- [x] AC-5: Page is admin-only (non-admins redirected)
- [x] AC-6: New user receives email with link to set password

---

## Tasks

- [x] T-23.1: Build server actions (createUser, deleteUser, updateRole) using service role client
- [x] T-23.2: Build Users page with table, create form, role toggle, delete button
- [x] T-23.3: Test create → invite email → login flow
