 ...
 */
export async function generateSimplePDF(form: FormData): Promise<Blob> {
  try {
...
export async function generateFormPDF(form: any, companySettings: CompanySettings): Promise<Blob> {
...
function generateFormContentHTML(form: any): string {
  let contentHTML = ""
  ...
  if (form.type === "Dust Control Log" || form.type === "Dust Control") {
    contentHTML = `
      ...
      <div class="form-section">
        ...
        <ul>
          ${form.content.dustControlMeasures?.map((measure: string) => `<li>${measure}</li>`).join("") || "None recorded"}
        </ul>
      </div>
    ...
    contentHTML = `
      ...
      <div class="form-section">
        ...
        <ul>
          ${form.content.bmpConditions?.map((condition: string) => `<li>${condition}</li>`).join("") || "None recorded"}
        </ul>
      </div>
    ...
    contentHTML = `
      <div class="form-section">
        <h3>Findings</h3>
        <ul>
          ${form.content.findings?.map((finding: string) => `<li>${finding}</li>`).join("") || "None recorded"}
        </ul>
      </div>
